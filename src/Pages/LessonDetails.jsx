...
export default function LessonDetails() {
    ...
    fetch("http://localhost:3000/lessons")
      ...
      .then((data) => {
        ...
        if (found) {
          ...
          scrollToTop(); // <-- scroll to top on lesson load
        }
      })
      .catch(err => {
  ...
  // ================= RENDER =================
  return (
    <section className={`min-h-screen transition-colors ${theme === "dark" ? "bg-gray-950 text-gray-100" : "bg-white text-gray-900"}`}>
      ...
      <div className="relative h-[60vh]">
        ...
        <div className="relative z-10 h-full flex items-end px-6 pb-10">
          <div>
            ...
            <h1 className="mb-10 text-4xl md:text-5xl font-extrabold text-white">{lesson.title}</h1>
          </div>
        </div>
        ...
        <div className={`p-5 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-800 ${theme === "dark" ? "bg-gray-900": "bg-white"} flex items-center justify-between gap-4`}>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm opacity-70">{lesson.creator.lessonCount || 'N/A'} lessons published</p>
            </div>
          </div>
          <Link to="/profile" className="text-indigo-500 font-medium hover:text-indigo-400 transition flex items-center gap-1 shrink-0">View profile →</Link>
        </div>
      </div>
      ...
      <div className={`px-6 py-14 ${isLocked && "blur-sm"}`}>
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-lg leading-relaxed mb-8 whitespace-pre-line">{lesson.fullDescription}</p>
          ...
          <div className="flex gap-4 mb-10">
            <button onClick={handleLike} className={`reaction-btn ${liked ? "liked" : ""}`} title={liked ? "Unlike" : "Like"}>{liked ? "💖" : "❤️"} Like</button>
            <button onClick={handleSave} className={`reaction-btn ${saved ? "saved" : ""}`} title={saved ? "Remove from Favorites" : "Save to Favorites"}>{saved ? "✅ Saved" : "🔖 Save"}</button>
            <button onClick={handleShare} className="reaction-btn">🔗 Share</button>
            <button onClick={() => setReportOpen(true)} className="reaction-btn text-red-500 hover:text-red-400">🚩 Report</button>
          </div>

          {/* COMMENTS */}
          <h3 className="text-xl font-bold mb-4 border-t pt-6 border-gray-200 dark:border-gray-800">Comments ({comments.length})</h3>
          <form onSubmit={handleComment} className="mb-8 flex gap-3">
            <input
              className={`p-3 rounded-lg shadow-sm flex-1 focus:outline-none focus:ring-2 focus:ring-indigo-500 ${theme === "dark" ? "bg-gray-800" : "bg-white"}`}
              value={commentInput}
              onChange={(e) => setCommentInput(e.target.value)}
              placeholder={user ? "Write a comment..." : "Log in to comment..."}
              disabled={!user}
            />
            <button className="px-5 py-2 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition disabled:opacity-50" disabled={!user || !commentInput.trim()}>Post</button>
          </form>
          <div className="space-y-4">
            {comments.map((c, index) => (
              <div key={index} className={`p-3 rounded-lg ${theme === "dark" ? "bg-gray-800" : "bg-white"} shadow-sm`}>
          ...
          {/* SIMILAR */}
          <h3 className="text-2xl font-bold mt-14 mb-6 border-t pt-6 border-gray-200 dark:border-gray-800">Recommended Lessons</h3>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {similarLessons.map((item) => {
              const locked = item.accessLevel === "premium" && !user?.isPremium;
              return (
                <div
                  key={item._id}
                  onClick={() => {
      ...
      {/* ================= REPORT MODAL ================= */}
      {reportOpen && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 transition-opacity" role="dialog" aria-modal="true">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl w-80 max-w-[90%] shadow-2xl transform scale-100 transition-transform">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setReportOpen(false)} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-300 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition">Cancel</button>
              <button onClick={submitReport} className="px-4 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-semibold disabled:opacity-50" disabled={!reportReason}>Submit Report</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl">
            ...
            <div className="flex flex-col gap-3">
              ...
              <a href={`https://wa.me/?text=${encodeURIComponent(lesson.title + " - " + window.location.href)}`} target="_blank" rel="noopener noreferrer" className="text-green-500 hover:text-green-400">💬 WhatsApp</a>
              <button onClick={() => { navigator.clipboard.writeText(window.location.href); toast.success("Link copied!"); setShareOpen(false); }} className="text-gray-500 hover:text-gray-400 text-left">📋 Copy link</button>
            </div>
            <button onClick={() => setShareOpen(false)} className="mt-6 text-sm bg-gray-200 dark:bg-gray-700 px-4 py-1 rounded">Close</button>
          </div>
        </div>
      )}
